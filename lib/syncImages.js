...
var syncDown = function(done) {
    ...
    var args = {
        url: this.options.api + (this.options.api.substr(-1) !== '/' ? '/' : '') + this.options.screenshotRoot + '.tar.gz',
        headers: { 'accept-encoding': 'gzip,deflate' },
    };

    if(typeof this.options.user === 'string' && typeof this.options.key === 'string') {
        args.auth = {
            user: this.options.user,
            pass: this.options.key
        };
    }
    ...
    r.on('response', function(resp) {
         ...
         * clear it if yes
         */
        if(fs.existsSync(self.options.screenshotRoot)) {
            rimraf.sync(self.options.screenshotRoot);
            fs.mkdirsSync(self.options.screenshotRoot, '0755', true);
        }
...
var syncUp = function(done) {
    var screenshotRoot = this.options.screenshotRoot,
        args = { url: this.options.api },
...
module.exports = function() {
  console.log(require('path').basename(__filename));
    ...
    return new Promise((resolve, reject) => {
        if(!this.options.api) {
            return reject(new Error('No sync options specified! Please provide an api path and user/key (optional).'));
        }
