...
try {
     ...
     * prepare page scan
     */
    const pageInfo = (await this.instance.execute(getPageInfo)).value
    ...
    if (CAPS_TAKING_FULLSIZE_SHOTS.indexOf(this.browserName) === -1) {
        const screenshot = await this.instance.saveScreenshot()
    ...
    while (currentXPos < (pageInfo.documentWidth / pageInfo.screenWidth)) {
         ...
         * scroll to next area
         */
        await this.instance.execute(scrollFn,
